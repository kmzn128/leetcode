...
public:
    ...
    SVI combinationSumUtil(VI &nums, int t, VSVI &memo) {
        SVI ans;
        VI new_nums;
        VI conums;
        for(int &i : nums) {
        ...
        int N = conums.size();
        REP(i, N) {
            SVI sub_ans = combinationSumUtil(new_nums, conums[i], memo);
    ...
    vector<vector<int>> combinationSum(vector<int>& candidates, int target) {
        ...
        SVI ans = combinationSumUtil(candidates, target, memo);
        int N = ans.size();
        VVI vans;
...
int main() {
    ...
    int T;
    cin >> T;
    VVI out = Solution().combinationSum(cand, T);
    for(VI & v : out) {
        int len = v.size();
        REP(i, len) cout << v[i] << " ";
        cout << endl;
